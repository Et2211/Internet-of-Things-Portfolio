...
if wifi.sta.status() == wifi.STA_GOTIP then
  ...
  --assign client to a role, in order of user, left, right
  conn:send("connected\n")
  if user=="none" then
    user = conn
  ...
  if conn==user then
    ...
    if s=='forehands' and left~="none" then
    ...
    elseif s=='backhands'and right~="none" then
    ...
    print("unauthorised user. You must be the user to send commands")
  end
  end)
end
)
